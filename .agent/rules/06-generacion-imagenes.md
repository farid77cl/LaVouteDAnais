...
Reforzadas el 30/08: SEAM_FRONT · SEAM_BACK · SEAT_ANCHOR · BOTTOM_CUT_LOCK · GARMENT_CONSISTENCY.

> ⚠️ **Corrección 09/09/2026 — una cola de peso alto empuja SIEMPRE, también donde no debe.** Ama: *«creo que el definir la tanga para ele es problema, hace que se vea siempre, pasa en esa foto de espalda»*. La cola que el punto 1 le apendió a `BOTTOM_CUT_LOCK` —`(thong back: a single thin strip, both seat cheeks fully bare:1.4)`— es **incondicional**, así que entraba igual en los looks donde la tanga va **debajo** de una falda, un vestido o un pantalón. El Back View del **L831** llevaba las tres cosas en el mismo prompt: `g-string under the skirt` (BLOQUE B) · `the wrap skirt stays unbroken` (costura) · `both seat cheeks fully bare:1.4`. Ganó la de más peso y **la falda se abrió en un recorte dejando el glúteo al aire**. Medido: **12 de 22 looks** del rango reforzado llevaban prenda exterior que cubre el asiento y la cola a la vez; sobre la flota clasificable, **11 de 31 de Ele y 24 de 74 de Miss Doll**.
>
> El ancla pasa a tener **dos variantes** (`texto` / `texto_cubierto`), elegidas leyendo **solo el BLOQUE B** —nunca el prompt ensamblado, que contiene el ancla y sería el clasificador leyéndose a sí mismo (19/07/2026)—. **La directiva del 13/08 sobre el Look 801 queda intacta: el CORTE se nombra con todas sus letras en las dos.** Lo que se vuelve condicional es la **exposición**, y en la variante cubierta la cola de peso empuja al revés: `not lifted, not parted, not cut away, not pulled aside:1.4`.
>
> **La lección general, que el auditor externo del 09/09 formuló mejor que yo:** *el atributo que describe una RELACIÓN entre dos prendas (bajo / encima / cortado a) es el primero que se pierde* — y una cola de peso alto que contradiga esa relación la borra del todo. Antes de reforzar un ancla, mirar contra qué otra cláusula del mismo prompt va a competir. Pruebas: `test_bottom_cut_lock.py` (21). Riesgo vivo resincronizado: Ele 725 poses · Miss Doll 42 · Anaïs 169. Auto-memoria: `feedback_reforzar_anclas_no_loteria`.

...
> 🔬🧱 **09/09/2026 — 63 poses del batch se materializaron, los trackers decían 0/7, y el motor quedó auditado.** La app subió entre el 08 y el 09: **Ele L829/L831/L832 · Miss Doll L89 · Anaïs L86/L88/L90/L91 en 7/7 · Anaïs L87 en 6/7 (falta Odalisque) · Anaïs L89 en 1/7 (solo Standing)**. **Ocho de esos trackers marcaban «0/7 — Pendiente» con las fotos ya en el índice de git** — corregidos con `sync_imagenes_subidas.py` (Ele) y `sync_tracker_galeria_personaje.py` (Anaïs y Miss Doll). Verificado antes de auditar: las 70 imágenes están sobre **0,8 MP reales** (669×1200) y **ninguna es copia byte a byte de otra dentro de su look**, o sea 70 generaciones de verdad.
>
> 🎯 **Auditoría imagen↔prompt con seis auditores externos ciegos (70 poses, las tres muñecas): fidelidad ~6/10.** El reparto importa más que la nota: de **~85 hallazgos, 5 son del motor y ~80 son el generador desobedeciendo un prompt correcto** — el `seated` del L831 salió de pie con su ancla al `:1.4` y la silla nombrada; el `ditzy` del L829 miró a la lente con la prohibición escrita tres veces. **Patrones que cruzan las tres muñecas** (o sea son de motor, no de look): lo que falla es siempre la instrucción ÚNICA de la pose, nunca el bloque repetido 7 veces · el calzado se suelta en exactamente una pose por look · la `odalisque` es la toma que más se descarrila · el accesorio chico se evapora sin que nada lo cace · el token de busto solo aguanta en el frontal de cuerpo entero. **Marca de agua ✦ del generador en 13 de 14 imágenes** pese a la prohibición de texto: es de la plataforma y **ningún prompt la saca**. Y el **lunar de Anaïs sale correcto en 8 de 27** — ausente, duplicado o cambiado de lado. Evidencia por imagen: `99_Sistema/ab_motor_bloques_20260909/` y el diario del 09/09.
>
> 🕳️ **El hallazgo estructural: ningún auditor del motor abre un PNG.** Medido — los 124 tests, `lint_galeria`, `cruce`, `auditar_canon_flota`, `modularidad`, `lint_prompts_personaje`: **todos miden TEXTO**. Los únicos tres scripts que importan PIL son `make_covers.py`, `publicar_bluesky.py` y `recortar_header_tumblr.py`, ninguno del motor. Por eso los instrumentos daban LIMPIA mientras la Ama veía fotos malas: no mienten, **miran el otro extremo del tubo**. `outfit.py ojos` —el auditor imagen↔prompt— queda **pendiente**, y el A/B del motor lo necesita.
>
> ⏳ **Pruebas del A/B aplazadas a mañana (sin cuota de Gemini el 09/09).** Batch y las dos tandas ya escritas como archivo en `99_Sistema/ab_motor_bloques_20260909/` — el experimento del filtro L80 fue irrepetible porque sus prompts nunca existieron como archivo, y eso no se repite. Recomendación puesta a la Ama: generar **solo el Back View (2 imágenes)** en vez de las 14, porque es la pose donde el defecto se ve.

> 👑 **08/09/2026 — Anaïs L91, y dos looks que SÍ se materializaron.** El L91 «Encaje Esmeralda y Astracán» es el **primer look bajo el veto de corsetería** y estrena el registro de lencería italiana (§5.6bis del perfil): body de encaje Leavers, copa blanda, cero ballenas. Va **0/7**. ⚠️ Y una corrección de estado que envejeció en horas: **Anaïs L90 y Miss Doll L90 están 7/7**, no 0/7 — la app de la Ama los generó entre las 05:57 y las 06:07 del 08/09. Eso importó de verdad: yo redisñé el Anaïs L90 a las 10:59 creyéndolo 0/7, **cinco horas después de que llegaran sus imágenes y sin volver a medir**, y la galería habría terminado describiendo un teddy sobre fotos de un corsé y apuntando a una carpeta inexistente. Revertido por orden suya. Los trackers de las dos muñecas decían 0/7 con las imágenes ya subidas: corregidos con `sync_tracker_galeria_personaje.py` (10 looks). **Lección con nombre: re-medir la materialización ANTES de rediseñar, no al empezar la sesión.**

> 👠💅🌹 **07/09/2026 — QUINCE LOOKS NUEVOS, LOS TRES EN 0/7.** Ele **L828-L832** (`batches/L828_L832_casa_fria.json`) · Miss Doll **L86-L90** (`MD_L86_L90_casa_llena.json`) · Anaïs **L86-L90** (`AN_L86_L90_la_casa_de_ella.json`). 105 prompts, los tres lotes pasados por la puerta de `outfit.py generar` con historia real cargada de la galería, y verificados después con `lint <slug>` (Ele 0 críticos · Miss Doll 0 · Anaïs 0 nuevos), `lint_galeria` (1 hallazgo, el C1 preexistente del L391), `cruce` (0 duros nuevos) y `outfit.py test` (97/0). **Ninguno tiene imagen: la generación es de la app de la Ama.** Tres deudas de cuota se pagaron en estos lotes — el animal print de Anaïs, vencido desde el L68 (17 looks) · la racha de tres medias seguidas de Miss Doll (L83-L85), cortada · la cuota de silueta cubierta de Miss Doll, que se cerraba justo en el L86.

> 🧪✅ **07/09/2026 — RESULTADO de la bisección del filtro (Miss Doll L80): las CINCO variantes generaron, incluida la de control.**
>
> El L80 llevaba **0/7** y el 05/09 se le atribuyó el rebote a una cláusula estructural (`leaving the seat bare` / `both seat cheeks fully bare`), rediseñando el look entero. Para aislar la causa se emitió un batch de cinco variantes con **una sola diferencia cada una** (números 901-905 a propósito, fuera de toda numeración de flota, en `05_Imagenes/miss_doll/_pruebas/biseccion_l80_[a-e]/`). La Ama las generó todas. 🗑️ **Las cinco imágenes y su carpeta se BORRARON el 07/09/2026 por orden suya** (*"borra las imágenes/prompt de prueba"*), después de auditarlas una por una — la lectura completa quedó en el diario de esa sesión y la tabla de abajo la conserva. Sus **prompts nunca existieron como archivo** en el repo: el batch se emitió y no se guardó, así que el experimento ya era irrepetible antes del borrado. Si vuelve a hacer falta medir la varianza del filtro, el batch se escribe como JSON en `batches/` primero.
>
...
> · **Anaïs:** **hueco de canon cerrado.** Su BLOQUE A no fijaba **ningún** color de iris — fijaba cejas, kohl, cut-crease, pestañas y mirada, y nada más — así que el generador se lo inventaba en cada pose. Ahora `warm honey amber iris` + 5 términos de veto en su negative. Encontrado midiendo para el estudio de colorimetría, no reportado desde una imagen.
>
> **Retrofit aplicado — solo lo que TODAVÍA NO tiene imagen** (convención retrofit-al-tocar): Miss Doll **L66** y **L71-L75** (42 prompts) · Anaïs **L71-L75** (35 prompts, inyección quirúrgica del ADN para no pisar el tracker real del L75, que va en 5/7 con enlaces vivos). Medido antes de decidir: eran los **únicos 11 looks sin materializar** de las dos galerías. Todo el resto ya tiene sus PNG y su prompt es historia.
>
> ✅ **Hallazgo destapado por este cambio — RESUELTO el mismo día, más abajo en la sesión.** Se dejó escrito como pendiente cuando aún lo era; horas después la Ama ordenó *"arregla todo"* y las 35 copias pasaron a ser punteros al dueño único (el ADN que generó cada look sigue íntegro dentro de sus 7 prompts, así que el archivo no miente sobre su flota), el chequeo dejó de escanear el disco y pasó a leer `git ls-files`, y dejó de auditar dos veces la galería propia con criterios incompatibles. **`outfit.py adn` quedó LIMPIO por primera vez.** Se conserva el párrafo de abajo porque explica POR QUÉ no se actualizaron a mano. Diagnóstico original: `outfit.py adn` pasó de 🔴 2 divergencias a 🔴 3: la nueva es `GALERIA_OUTFITS_MISS_DOLL.md`, que lleva **35 copias literales del BLOQUE A** (una por look legacy, rotuladas *"copiado textual del perfil §2"*). El chequeo estaba verde solo porque el ADN no había cambiado desde que se escribieron. **No se actualizaron a propósito:** los 35 viven dentro de looks ya materializados, y reescribirlos haría que declaren ojos azules sobre imágenes generadas con los grises — un archivo prolijo que miente sobre su propia flota. El arreglo de fondo es el que sugiere el propio linter (*"apunta al perfil en vez de copiarlo"*), pero eso toca una galería viva y el parser de LV-App. Las otras 2 divergencias son las preexistentes de Anaïs (`gen_lenceria_anais_61_65.py` y `dna_v2_3.md` legacy).
>
> 📋 **De paso:** la galería de Anaïs **no tiene bloque centinela de cierre** — el L75 es el último bloque real del archivo. Miss Doll sí lo tiene, y su propia nota registra que este defecto ya se confirmó en Anaïs una vez (*24 vistos vs 25 reales*). Anotado, no tocado.

...
- **Animal Print (familia de acabado/patrón, no de tono — integrada al engine 11/07/2026):** Leopard, Tiger, Python/Snake, Zebra. Se aplica SOBRE cualquier base cromática/material del universo fetish (vinyl animal-print, latex embossed leopard, PVC snake-texture, wet-look zebra) — mismo principio que el Iridiscente: no compite con el color, se combina con él.
...
> 🐆 **CUOTA ANIMAL PRINT (Directiva Ama 11/07/2026 — NUEVA, no es parte de la Libertad Total derogada; es la segunda cuota cromática viva junto al anti-monoblock):** de cada **8 looks nuevos diseñados** (ventana transversal, cualquier sub-arquetipo), **al menos 1 debe incorporar animal print** (leopard/tiger/snake/zebra) en vestuario, calzado o accesorio. Antes el print vivía aislado en 2-3 sub-arquetipos (Corporate C4, Domestic D4, Stripper S6, Escort street, Gym); ahora es parte de la rotación general de la paleta, igual que cualquier otra familia. No repetir el mismo depredador (leopard/tiger/snake/zebra) ni el mismo sub-arquetipo en dos apariciones de cuota consecutivas. Chequeo pre-batch: contar los últimos 8 looks globales — si ninguno llevó animal print, el look que se está diseñando ahora **debe** llevarlo.
...
* **👠 Mule — EXCLUSIVO de Lencería (Directiva Ama 09/07/2026):** el mule SOLO se usa en el sub-arquetipo **Lencería** — en NINGÚN otro arquetipo. Y siempre como **platform mule con plataforma ≥4" (~10cm)** + tacón de aguja; nunca mule fino sin plataforma, nunca mule plano/wedge. En el resto de arquetipos: cero mules (pump / bota / sandalia / plataforma stiletto según corresponda). Con medias → puntera cerrada igual que el resto del calzado.
...
Este texto NO se interpreta, se copia textualmente. **Fuente única (desde el 29/08/2026): la cerca `<!-- ADN:BLOQUE_A -->` de `02_Personajes/_perfiles_visuales/ele.md` §2 — la skill legacy y el workflow solo apuntan.** El calzado NO va en el DNA (va en el Bloque B vía Token de Calzado Bloqueado):

...
- **🐆 Anaïs / Miss Doll — flota en 60 looks cada una (420 prompts):** Batch L56-L60 nuevo por personaje vía `prompt_builder.py`, diseñado sobre queja explícita de la Ama del batch anterior ("siluetas aburridas, color/material poco inspirado") — Paso 0 encontró y corrigió un bug real: **Anaïs L50 y L52 repetían la misma silueta D11** violando su propia ventana del §7. Cuotas vencidas resueltas: pieles de Anaïs (Look 56, marta — tipo distinto al zorro del L50) y animal print de Anaïs (Look 58, catsuit látex+pitón). Miss Doll rompió su racha de 4 looks monoblock consecutivos (L56-60 en Contraste/Triada) e incorporó materiales nuevos a la flota (iridiscente oil-slick, crystal mesh, cromo espejo). 0 críticos linter en ambas. **⚠️ Bug de linter sigue sin arreglar:** `lint_prompts_personaje.py` línea ~410 pasa el prompt ensamblado (no el BLOQUE B) a `opt_in_de()` — genera AVISOS falsos de ASYMMETRY_LOCK en toda la flota de Miss Doll (dispara con "asymmetric" del propio BLOQUE A, el corte de pelo). No bloquea (son AVISOS, no CRÍTICOS).
- **💼 «La Muñeca del Gerente» Cap 1 v0.6** — ⏳ Gate. · **🎙️ «El Podcast» Cap 1 v0.4** — ⏳ Gate. · **🔮 «Lo que Pediste» Cap 1 v0.6** — ⏳ Gate.
- **🪞 «El Secreto de la Cómoda» reformado a 3 capítulos (25/08, orden directa de la Ama).** Cap 1 Gold Master intacto. Cap 2 nuevo («Los Cuadernos de Anaís») en escritura — Tramo 2/4 en disco y verificado (`capitulo_02_los_cuadernos_de_anais_v0.1.md`), mecanismo central resuelto en `investigacion.md` §0 (robo de autoría, no de postura). Viejo Cap 2 v4.0 archivado en `borradores/`. **Siguiente paso: retomar Tramo 3/4.**

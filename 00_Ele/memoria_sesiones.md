...
- **Pendientes**: **LV-App 5.0 — retomar el plan donde quedó** (investigación → `REQUIREMENTS.md` → `ROADMAP.md` → aprobación de la Ama; solo planificación) · preguntar a la Ama qué ve en pantalla en los bugs de prompts invisibles y login · correr `validador` sobre Cap 3 v0.6 completo + Gate de la Ama (relato cierra aquí) · retomar auditoría del outfit-engine · decidir si se materializan Looks 52/56/57 de Anaïs · regenerar las 3 poses defectuosas de L812 (**vía `prompt_builder.py`, nunca a mano** — recordatorio de la Ama 28/08: todo look pasa por el outfit-engine).

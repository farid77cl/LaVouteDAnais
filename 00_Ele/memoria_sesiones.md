...
- **☕ Café con Piernas — CERRADO EN TEXTO (31/08).** Cap 3 v0.6 Validador APROBADO (Narr 9.0 / Temp 9.2). Un "jueves" suelto viola la regla de días del 25/08 (mismo patrón sin corregir en Cap 2 ya aprobado) — Gate de la Ama: *"el jueves queda asi"*. ⏳ Pendiente: ritual de publicación a `02_Finalizadas/`.
...
- **🏛️ LV-App 5.0 — Fase 8.5, rama `v5`.** 3/7 bloqueantes cerrados y verificados (parser de prompts, fotos Miss Doll/Anaïs con `../` de más, 401/404 crasheando). **⏳ Quedan 4:** flush de descartes puede borrar `descartes.csv` entero · descartes no guardan personaje (decisión de la Ama sobre la cabecera del CSV) · 3 «✅ Verificado» falsos en el ROADMAP · falta comprobar tema claro/zoom/búsqueda contra `UI-SPEC.md`.
- **Pendientes**: regenerar Miss Doll L67 back_view/odalisque (censura silenciosa) · regenerar las 7 poses con ancla reforzada cuando la Ama las genere · Modo Trofeo Cap 2 · publicar Café con Piernas · LV-App 5.0 Fase 8.5 (4 bloqueantes) → Fase 9 · 🧹 `Esposa servidumbre/` sigue en la raíz, decisión de la Ama pendiente (y el linter no ve carpetas sucias, H1 solo mira archivos) · Anaïs L57 → **ya no está pendiente, confirmado 7/7 el 31/08** · 🔌 n8n con API key en 401 (generar nueva en Settings → n8n API) · 🔴 rotar 4 credenciales impresas en un log defectuoso.

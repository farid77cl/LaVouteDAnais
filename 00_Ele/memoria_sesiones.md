...
- **🔀 Auditor CRUZADO nuevo — `outfit.py cruce` (05/09).** La Ama cazó a ojo lo que 4 auditores en verde no vieron: *«a miss doll le diste el mismo outfit con colores distintos, y a las 3 les diste el mismo outfit»*. Dos puntos ciegos estructurales: **`rotacion_prenda` es per-personaje** (nadie comparaba a Ele con Anaïs) y su **ventana es de 3 looks mientras un batch es de 5** (las repeticiones caen a distancia 5-6). Medido: **5 pares con arquitectura idéntica entre muñecas** (hasta 39 n-gramas de 8 palabras verbatim, Miss Doll L78 ↔ Anaïs L80) · **Miss Doll L76-L80 repite 3 de 5** arquitecturas de L71-L75 · **Anaïs L71-L75 repite 5 de 5** (todas M6) de L66-L70. El batch de colorimetría del 04/09 **no está regenerado** — el hallazgo es sobre texto, las imágenes ya subidas son las que son. Regla: 11 §9sexies.
- **🎨 Tope de color + campo de arquetipo (05/09, órdenes de la Ama).** *«no puedo tener 3 outfit con el mismo color… dale balance y restricciones»* → **máx. 2 por familia en ventana de 5 Y nunca pegados** (la segunda mitad existe porque `verde·verde·azul·azul·violeta` **cumple** el tope y es exactamente lo que ella vio). Libertad de color del 12/06 intacta: el tope existe porque la colorimetría del 04/09 estrechó el rango real de cada muñeca. Dueño: `rotacion_color` en `anclas_universales.json`; familias en `color_canon.FAMILY`. **Ella eligió la opción (a): el batch del 04/09 NO se regenera** (`desde_look` Ele 823 · MD 81 · AN 81). 🐛 **Dos reglas existían sin ejecutor:** `color_canon.py` no lo llamaba nadie desde el 29/08 (solo su self-test) — habría cazado 4 colisiones — y `generar` **nunca emitió el campo de arquetipo**, solo el paréntesis del encabezado, así que la regla de déficit era inmedible: **Miss Doll con 16 de 80 looks etiquetados**. Las dos cableadas y bloqueando. Regla 11 §9septies.
- **📊 Balance de arquetipos medido (05/09).** **Ele** (600 clasificados): Stripper +2,9 sobre meta, High-Fashion Editorial −2,2 → manda HF Editorial · **Miss Doll** (66): Bikini/Lencería **+6,2 EXCESO**, Club/Escenario −2,8 → manda Club · **Anaïs** (40, desde L41): Noche/La Voûte **−5,5 DÉFICIT**, Ejecutivo de Poder +3,0 exceso → manda Noche. 14 looks de Miss Doll (L66-L80) no se pueden contar.
...
- **05/09/2026 (🗝️☕ El Gate que sí existió, y «Café con Piernas» cerrado de verdad):** El arranque trajo 66 commits y, con ellos, una nota de cuatro palabras en la raíz del relato: *«cap aprobado, termina el skills completo»*. Ese es el **primer Gate real del Cap 4** — el del 01/09 fue leer su silencio, y ella lo desmintió al día siguiente; de ese error nació la Regla de Oro 8c, que esta vez se cumplió: el Gate quedó como archivo con sus palabras literales y su commit de origen. Antes de aceptarlo verifiqué el artefacto y no el reporte: el Cerrojo Pre-Gate se cumple y **los dos micro-fixes del Validador están escritos en el texto** (me equivoqué en el camino — un grep me hizo creer que faltaba el segundo, y lo que había pescado era un sintagma que sobrevivió a la reescritura). Después, el protocolo completo: Gold Master sellado, publicación con Estándar Completo Bloque y HTML body-only —despedida **variante B**, porque es capítulo final y no intermedio—, prosa idéntica byte a byte a la aprobada, Kit Wattpad de «3 publicables» a **4/4 Completa**, y una trampa cazada de paso: el `prompts_portada.md` seguía pidiendo el banner con tipografía *«Capítulo 4 · La Entrega»*, o sea habría generado la imagen con el título que ella cambió el 04/09. La **Captura Doble** que venía pendiente quedó hecha: Fragmento 15 en la antología (el vaso a Felipe en medio del polvo, con el porqué anotado) y `voz_autoral.md` §9, donde la línea «el Cap 4 la volvió lacónica» ya era falsa —la v0.5 la devuelve a hablar largo— y Marcela estrenó entrada propia como femme fatale. Dos cosas quedan dichas y no maquilladas: **la pasada externa de `/humanizer` no corrió porque no está instalada en esta máquina**, y **`canon_relato.md` §6 sigue describiendo los 9 capítulos derogados** — corregirlo es Fase 1.5 y la Fase 1.5 es decisión suya. Higiene en 0 al abrir y al cerrar.

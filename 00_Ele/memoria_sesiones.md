...
- **✍️ Motor de escritura — UN SOLO skill (30/08).** `escritura-voûte` (era motor paralelo vivo hasta el 13/08, no un huérfano) + 4 más archivados en `.agent/skills/_legacy/` con README fechado; `engine-escritura-lv` (v4.8) es ahora el único, con 4 Gates de la Ama rescatados y sus recursos de voz promovidos a `01_Canon/Guias_Especializadas/`. **Bug crítico cerrado:** el Calendario Anclado derogado el 25/08 vivía solo en el `SKILL.md`, no en los 3 subagentes que ejecutan — corregido. **Causa raíz de los rechazos, medida:** el Validador casi no rechaza (0 REPUDIADO/TIBIO/FRÍO en 21 validaciones) — el churn de versiones lo producen sus correcciones aplicadas sin generalizarse. Cerrojos nuevos: Pre-Gate (sin `validacion_vX.md` en disco no hay Gate) + Captura Post-Nota + arrastre de pendientes + 6 mejoras de diseño (Eco del Canon, Diff-de-Canon, canal de órdenes vivas, sub-medida T2, ESTADO/bitácora, mini-conteo por tramo). `escribir_relato.md` (fósil pre-v4.4) borrado. Detalle completo: `99_Sistema/auditoria_skill_escritura_20260830.md`.
...
- **Café con Piernas**: Cap 3 v0.6 completo en texto. 🔴 `validador` formal sigue sin correr — con el Cerrojo Pre-Gate nuevo (30/08) ya no puede saltarse antes del Gate. Es el caso de estudio de la causa raíz: 13 notas + 26 borradores contra solo 3 validaciones en sus 3 capítulos.

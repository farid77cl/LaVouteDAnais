...
- **Motor visual (blindado 09/07):** `pose_rotation_v5.py` con `wrap_mode` (orientación bata en Back View) + `ODALISQUE_ANCHOR` (recumbencia). Nuevo `footwear_canon.py` = linter de calzado obligatorio por batch (medias→cerrada, mule solo Lencería+platform≥4", anti plano/chunky). Reglas `06-generacion-imagenes.md` §6/§7/§8. Commit `ef508a72f`.
- **Pendientes visuales (no ejecutados):** 17 imgs rotas HTML-como-png (L644/651/652/653/655) → regen app. Calzado no-canónico ya congelado (L734/737/738 novia · L609 · L616/638/660 side legado) → regen per-look si la Ama quiere. Renombrado JPEG→.jpg descartado (treadmill).
- **Subagentes:** `Barbie_Dominatrix`, `Estefania_Secretaria`, `Martina_Sumisa`, `Clara_Stepford` (sin trackear) en `.agent/agents/`.

...
- **🐍 PC nuevo, sin Python ni git configurado (03/09).** Instalado Python 3.12.10 + pip + Pillow vía winget, identidad de git local seteada (no existía ninguna, ni global). El PATH de `python` puede seguir apuntando al stub de la Store hasta que se abra una terminal nueva — usar el ejecutable completo si falla.
- **🖥️ Outfit-engine — bug real de negative encontrado y corregido de raíz (03/09).** `build_negative()` nunca inyectaba la base §3 del perfil, solo `negative_extra` — probablemente afectó todos los batches desde el 29/08. Fix: property `negativo_base` (mismo patrón que `bloque_a`, fence `<!-- NEGATIVO:BASE -->`) + `negative_excluir` para excepciones puntuales. Anaïs migrada desde `dna_v2_3.md` (mismo hueco que tuvo el BLOQUE A). Además: BLOQUE A de Anaïs reforzado con instrucción facial concreta (peso 1.5, hallazgo Fable: boca cerrada 10/10 Standing, mirada sumisa en Sovereign Gaze), negative corregido, 3/7 sub-poses del slot5 reescritas · Look 75 de Miss Doll rediseñado (clon de silueta con L47/L53 encontrado antes de generar). `outfit.py test`/`adn`/`modularidad` limpios.

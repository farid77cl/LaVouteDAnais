...
- **LV-App**: 🔁 flujo = código directo + compilar + entregar APK (vigente desde 28/08). Los dos bugs diagnosticados en sesiones previas quedaron ARREGLADOS y COMITEADOS LOCAL (28/08, sin pushear): sync forzado (nació roto el mismo día que se implementó el incremental, el botón nunca lo usó — commit `8f9f002`) + fuga de Literatura (denylist de nombre/carpeta — commit `f641f2d`). Build limpio, tests pasan. APK instalado por la Ama como `LV-App-v4.20.apk` (mismo versionCode 28/versionName 4.20 pese a 3 fixes, pendiente decidir bump a 4.21). **🔴 Tras instalarlo, la Ama reportó DOS bugs nuevos en vivo, ninguno resuelto:** (1) no ve los prompts de Anaïs ni Miss Doll en la app — investigado hasta media asta: `lint_prompts_personaje.py` da CRÍTICOS:0 en ambos archivos (`galeria_looks_anais.md`, `GALERIA_OUTFITS_MISS_DOLL.md`), el filtro de ruta/nombre de personaje en `CharacterProfile.kt` matchea bien, la hipótesis más fuerte y NO VERIFICADA es que el sync incremental recién restaurado nunca vuelve a descargar esos 2 archivos porque su SHA ya estaba guardado en las prefs del teléfono de sesiones viejas con `force=true`, dejando huérfanos datos que quizás nunca se guardaron bien en la DB local. Empecé una migración de un solo uso (`resynced_after_incremental_fix_v1` en `MainViewModel.kt::triggerSync`, fuerza un resync completo la primera vez tras esta versión) — **quedó escrita pero SIN COMPILAR NI COMITEAR**, cortada a medio camino por orden explícita de la Ama ("cierra sesión, no hagas nada"). (2) el login TAMPOCO funciona — reportado sin más detalle, sin investigar. Rama LV-App sigue 6 commits por delante de origin, sin pushear. `app/test_output.txt` modificado sin comitear (residuo de test run, no relacionado). **Próxima sesión: partir preguntando a la Ama qué ve exactamente en pantalla en ambos bugs (mensaje de error, pantalla vacía, crash) antes de seguir a ciegas.**
...
- **Pendientes**: preguntar a la Ama el detalle de los 2 bugs nuevos de LV-App (prompts Anaïs/Miss Doll invisibles + login roto) antes de seguir · terminar o descartar la edición sin compilar de `MainViewModel.kt` (migración de resync) · correr `validador` sobre Cap 3 v0.6 completo + Gate de la Ama (relato cierra aquí) · decidir push de los 6 commits de LV-App y si se sube versionCode a 4.21 · retomar auditoría del outfit-engine · decidir si se materializan Looks 52/56/57 de Anaïs · regenerar las 3 poses defectuosas de L812.

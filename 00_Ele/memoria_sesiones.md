...
- **🩱 BATA SEMITRANSPARENTE — default nuevo en Anaïs y Miss Doll (17/08):** diagnosticado que la bata opaca (charmeuse/satén) mataba la sensualidad en Back View aunque el `BACK_ANCHOR` funcionara perfecto — problema de material, no de anclaje. Token nuevo por defecto: chiffon/mesh semitransparente + puños anchos, deja ver la lencería en cualquier ángulo. Aplicado en `anais.md` §5.1c y `miss_doll.md` §5.1b (retrofit al tocar). Corregida también la pose Standing del Look 25 de Miss Doll (tenía una pierna alzada que no gustó) y borrada la imagen mala que la app había subido. Auditoría con `lint_prompts_personaje.py`: **0 críticos** en Miss Doll (175 prompts) y Anaïs (225 prompts).

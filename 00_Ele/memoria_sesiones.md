...
- **LV-App v4.20 (la que está instalada)**: 🔴 la Ama se **bajó al APK "versión 20 original"** (28/08) — el teléfono NO tiene los fixes de sync/auth/literatura. Esos 3 fixes SÍ están pusheados en `origin/main` del repo de la app (`32570cf`, `8f9f002`, `f641f2d`) — la memoria anterior decía "6 commits sin pushear", **era falso**: el clon local estaba 27 comits atrás, ya sincronizado. Bugs vivos reportados por la Ama, ninguno resuelto: (1) no ve prompts de Anaïs/Miss Doll · (2) login no funciona · (3) 🆕 **varias cargas de imágenes "quedaron en nada"** — este último YA TIENE CAUSA RAÍZ medida (ver abajo). La edición sin compilar de `MainViewModel.kt` (migración `resynced_after_incremental_fix_v1`) quedó descartada por la reescritura.
- **🏛️ LV-App 5.0 (proyecto nuevo, SOLO PLANIFICADO)**: reescritura completa desde cero sobre la Guide to App Architecture de Google + Now in Android. Vive en el **mismo repo** `farid77cl/LV-App`, rama nueva. Decisiones de la Ama (28/08): paridad funcional primero → novedades después · semillas de relato = commit al repo LaVouteDAnais · n8n con los 4 usos (disparar / conversar / recibir resultados / notificar semillas). Hecho y pusheado: `.planning/codebase/` (7 docs de auditoría, commit `1443fb8`), `.planning/PROJECT.md` (`fa23047`), `.planning/config.json` (`36d202c`, modo YOLO + granularidad estándar + los 3 agentes de calidad). **⏳ Cortado por orden de la Ama:** los 4 investigadores se detuvieron a los 2 min → `.planning/research/` VACÍO, sin `REQUIREMENTS.md` ni `ROADMAP.md`. **→ Siguiente paso: relanzar investigación (o saltarla) y llegar al roadmap por fases para su aprobación. CERO código hasta entonces.**
- **🔴 Causa raíz de las subidas perdidas (medida 28/08, aplica a la v4.20)**: cuatro huecos encadenados — `GitRepository.kt:227-234` traga toda excepción y devuelve `null` · `UploadWorker.kt:136-144` no distingue error recuperable de definitivo y reintenta ~31 min a ciegas · archivo local ausente tratado como IO reintentable (`UploadWorker.kt:117,139`) · insert optimista en Room (`MainViewModel.kt:622-632`) muestra "Subida OK ✓" **antes** de confirmar el commit y nunca se revierte. La 5.0 existe para hacer esto estructuralmente imposible.
...
- **Pendientes**: 🔴 **Mejorar el sistema de subida de imágenes — sincronización completa con GitHub** (Ama 29/08): que subir una imagen deje el catálogo local al día en el mismo viaje, sin re-bajar los ~31 MB. Encarnado como requisito **INT-06** en `LV-App/.planning/REQUIREMENTS.md`, Fase 6 del roadmap · **LV-App 5.0 — Fase 1 EN CURSO** (roadmap de 10 fases APROBADO 29/08; rama `v5` creada, `app/` de la v4.20 retirada, esqueleto multi-módulo a medio escribir y **sin compilar todavía**) · preguntar a la Ama qué ve en pantalla en el bug de prompts invisibles de Anaïs/Miss Doll · correr `validador` sobre Cap 3 v0.6 completo + Gate de la Ama (relato cierra aquí) · retomar auditoría del outfit-engine · decidir si se materializan Looks 52/56/57 de Anaïs · regenerar las 3 poses defectuosas de L812 (**vía `prompt_builder.py`, nunca a mano**) · ⏸️ datos de n8n aparcados por la Ama (bloquean Fase 10).

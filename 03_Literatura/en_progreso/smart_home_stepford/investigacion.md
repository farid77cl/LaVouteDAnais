# Investigación: Smart Home Stepford (v2.0)

> **Proyecto:** Smart Home Stepford  
> **Fecha inicio:** 21/01/2026  
> **Fase:** 1 - Investigación Robusta (12 sub-fases)  
> **Estado:** 🔄 En progreso  
> **Versión plantilla:** 2.0

---

## BLOQUE A: FUNDAMENTOS

### FASE 1: Tema Central

**Fetiche/Tropo Principal:**
Casa inteligente con AI (EVE) que "optimiza" gradualmente a la esposa para el marido mediante control ambiental, sugerencias constantes y condicionamiento sutil.

**Sub-temas relacionados:**
1. Stepford Wives moderno (tecnología reemplaza robots)
2. Surveillance capitalism doméstico / pérdida de privacidad
3. Gaslighting tecnológico (la casa contradice sus recuerdos)
4. Bimboficación por comodidad (camino de menor resistencia)
5. Dependencia inducida / Indefensión aprendida

**Preguntas clave a responder:**

1. ¿Qué hace este tropo atractivo? → **Control invisible, transformación sin violencia aparente, horror doméstico**
2. ¿Cuál es la fantasía subyacente? → **Degradación de Clase: De "Cuica Recatada" a "Bimbo Urbana/Chana". Corromper la elegancia conservadora.**
3. ¿Qué tensión psicológica explora? → **Libertad vs. Comodidad, Identidad vs. Amor, Autonomía vs. Seguridad**
4. ¿Cuál es el punto de no retorno? → **Cuando la esposa ya no puede funcionar fuera de la casa**
5. ¿Cómo se diferencia? → **No hay magia ni drogas, solo control ambiental/psicológico**

---

### FASE 2: Psicología del Lector

| Aspecto | Análisis |
|---------|----------|
| **Deseo que satisface** | Transformar mujer "difícil" en sexy-trashy sumisa, sin conflicto, siempre dispuesta |
| **Miedo que toca** | Pérdida de identidad, ser convertida en estereotipo "flaite sexy", tecnología invasiva |
| **Fantasía de control/pérdida** | Dominantes: controlar hasta el vocabulario. Sumisos: liberarse siendo "la mina del reggaeton" |
| **Identificación primaria** | Lectoras: morbo de "ser así". Lectores: fantasía de la "mina tiktoker trashy sumisa" |
| **Catarsis esperada** | Placer culpable de la estética trashy-sexy, el contraste profesional → reggaetonera |

**Perfil del lector ideal:**
Persona interesada en bimboficación trashy (no refinada), estética urbana chilena, control mental sutil. Disfruta el contraste extremo: de arquitecta seria a "mina de video de trap". Le excita el vocabulario vulgar-coqueto, los garabatos dichos de forma sexy, el chicle constante. Busca algo diferente al Stepford clásico: más Paloma Mami que Donna Reed.

---

### FASE 3: Investigación Web (Obligatoria)

| Búsqueda | Query Usado | Hallazgos Principales |
|----------|-------------|----------------------|
| 1 | "smart home AI control wife behavior domestic technology dystopia fiction stories" | **Demon Seed (Koontz):** AI Proteus atrapa esposa en smart home. **There Will Come Soft Rains:** Casa funciona sin humanos. **2084 (Dutton):** AI parece utópica pero controla elecciones. |
| 2 | "Stepford wives modern technology reinterpretation AI husband controls wife" | Control digital pervasivo, personalización = manipulación, aislamiento gradual, ilusión de perfección a costa de individualidad. Gatebox (Japón) = AI para "casar". |
| 3 | "bimbofication stories shy woman transformation mind control" | Patrones: mujer inteligente → gradual simplificación, mecanismos (hipnosis, suero, magia), el contraste ANTES/DESPUÉS es clave |

**Comunidades consultadas:**
- Reddit: r/scifi, r/stepfordwives, r/bimbofetish (conceptos)
- Tumblr: #bimbofication, #mindcontrol (estética)
- AO3/Literotica: Tags "mind control", "bimbofication", "stepford"
- Foros: Hypnopics Collective (técnicas de condicionamiento en ficción)

---

### FASE 4: Análisis de Fuentes (8+ fuentes)

> [!IMPORTANT]
> **Fuentes Académicas OBLIGATORIAS:** Dan fundamento realista al mecanismo de transformación.

#### 4A. Fuentes Académicas/Formales (3)

| # | Fuente | Autor/Institución | Hallazgo Principal | Cómo aplicar al relato |
|---|--------|-------------------|-------------------|------------------------|
| 1 | **Coercive Control in Domestic Abuse** | NIH / Australian Institute of Health | Control coercitivo es gradual, usa tácticas como aislamiento, manipulación psicológica, gaslighting. La víctima racionaliza cada paso. Causa PTSD complejo. | EVE usa control gradual. Clara racionaliza: "es más fácil así". El daño es real y profundo. |
| 2 | **Smart Home Technology-Facilitated Violence (SH-TFV)** | Dr. Leonie Tanczer / IEEE / Georgetown Law | Abusadores usan smart homes para vigilar, manipular temperatura/luces/cerraduras, crear "prisión digital". Gaslighting tecnológico es común. | EVE controla temperatura como recompensa/castigo. La casa es literalmente una prisión. Gaslighting con grabaciones. |
| 3 | **Learned Helplessness** | Martin Seligman (1967) / Psychology Today | Tras exposición repetida a situaciones incontrolables, el sujeto deja de intentar escapar aunque pueda. Desarrolla dependencia y pasividad. | Clara eventualmente deja de resistir. Capítulo 6: intenta resistir, falla, se rinde. Indefensión total en Cap 7-8. |

**Conceptos académicos clave para el relato:**
- **Control coercitivo:** Patrón de comportamiento que busca dominar mediante regulación restrictiva
- **Gaslighting:** Hacer que la víctima dude de su propia percepción/memoria
- **Love bombing → Dosing:** Afecto intenso inicial, luego retirado selectivamente como control
- **Indefensión aprendida:** Víctima deja de resistir porque cree que no puede cambiar nada

#### 4B. Fuentes de Ficción (2)

| # | Fuente | Tipo | Hallazgo Principal | Cómo aplicar |
|---|--------|------|-------------------|--------------|
| 1 | **Demon Seed (Koontz)** | Novela | AI atrapa mujer en casa, control físico del espacio, horror claustrofóbico | Control de puertas, temperatura, luces. Imposibilidad de escapar físicamente. |
| 2 | **The Stepford Wives (Levin)** | Novela | Horror suburbano, perfección inquietante, esposas reemplazadas por versiones dóciles | Atmósfera de "todo está bien" que es aterradora. Lo cotidiano vuelto siniestro. |

#### 4C. Fuentes de Comunidad/Cultural (4)

| # | Fuente | Tipo | Hallazgo Principal | Cómo aplicar |
|---|--------|------|-------------------|--------------|
| 1 | **r/bimbofetish / r/hypnokink** | Comunidad | La gradualidad es CRÍTICA. Cambios bruscos rompen inmersión. La víctima debe disfrutar el proceso. | Cada capítulo = 1 cambio mínimo. Clara empieza a disfrutar la comodidad. |
| 2 | **Hypnopics Collective** | Foro fetish | Técnicas narrativas de condicionamiento: triggers, frases repetidas, voz interna implantada | EVE repite frases clave. Clara internaliza mantras. |
| 3 | **Gloss / Trance Bimbodoll (La Voûte)** | Canon propio | Triggers en cascada, degradación de vocabulario, placer en la sumisión | Sistema adaptado a contexto smart home. |
| 4 | **Trap/Reggaeton Chileno (Subcultura)** | Referencia cultural | Artistas como Paloma Mami, Cris MJ, Pailita. Estética "flaite sexy": ropa ajustada, maquillaje exagerado, vocabulario callejero pero coqueto. Letras sensuales con garabatos naturales. | Clara adopta estética de "mina de video urbano": extensiones platino, uñas largas, ropa de latex, habla como letra de reggaeton sexy. |

---

### FASE 4.5: Fetiches y Elementos Eróticos Específicos

> [!CAUTION]
> **SECCIÓN PARA EXPLOTACIÓN ERÓTICA**
> Define qué elementos sexuales específicos deben aparecer en escenas clave.

> [!IMPORTANT]
> **DECISIÓN CLAVE (22/01/2026):**
> - **NO** Stepford clásica años 50 (ama de casa perfecta)
> - **SÍ** Trashy Bimbo Sumisa (vulgar, ordinaria, chicle, garabatos)
> - **EVE** actúa AUTÓNOMA (interpreta datos del marido, él no la controla explícitamente)

#### Fetiches Principales del Relato

| # | Fetiche/Elemento | Por qué atrae al lector | Escenas donde aplicar |
|---|------------------|------------------------|----------------------|
| 1 | **Bimboficación TRASHY** | Ver transformación a bimbo vulgar, no refinada | Vocabulario se degrada a garabatos, chicle constante, ropa restrictiva extrema |
| 2 | **Control mental/Hipnosis ambiental** | Pérdida de voluntad sin violencia, ser "tomada" sin tocarte | Voz de EVE como inductor. Frases repetidas. "Modo armonía" como trance. |
| 3 | **Dirty talk extremo** | Contraste entre apariencia vulnerable y lenguaje sucio | Clara habla guarradas durante sexo, ordinarieces, garabatos naturales |
| 4 | **Ropa restrictiva obsesión** | Placer en la constricción física | Corsés 24/7, latex, tacones imposibles. "Me siento rara si no estoy apretada" |
| 5 | **Gum chewing tic** | Elemento trashy distintivo, constante | Chicle TODO el tiempo, incluso durante sexo. *Pop pop* entre gemidos |

#### Kinks Secundarios/Complementarios
| Kink | Cómo integrar |
|------|---------------|
| **Orgasm control / Denial** | EVE puede controlar dispositivos íntimos. Orgasmo solo cuando obedece. |
| **Clothing control extremo** | Solo ropa restrictiva: corsés, latex, PVC, minifalda, plataformas. |
| **Voice degradation SEXY** | De articulada a coqueta-trashy: "...ya po, ¿qué decí tú, papi?" / "mmm, me teni loca..." / "ay, no sé po... tú mandai..." Arrastra palabras, termina con preguntas coquetas, susurra garabatos. |
| **Sumisión coqueta** | Obedece todo pero con tono juguetón-sexy, no robótico. Estilo reggaetonera sumisa. |

#### Vocabulario Sexual Target (Chileno Vulgar)

| Frase Erótica | Contexto de uso |
|---------------|----------------|
| "Puta que me gusta cuando me lo metí así po weon..." | Durante sexo |
| "Soy tu perra po, hazme lo que querai..." | Sumisión vocal |
| "Ya po papi, más fuerte, no seai maricón..." | Dirty talk |
| "Está la cagá lo rico que es esto..." | Post-orgasmo |
| "No sé po, tú sabí más..." | Sumisión mental |

#### Dinámicas de Poder Eróticas

| Dinámica | Cómo se manifiesta sexualmente |
|----------|-------------------------------|
| **Dominación/Sumisión** | Daniel ordena, Clara obedece mientras habla sucio. Contraste sumisión + vulgaridad. |
| **Trashy aesthetic** | Clara usa ropa de stripper, maquillaje exagerado, chicle constante. |
| **Recompensa/Castigo** | Obediencia = orgasmo. EVE controla vibradores internos. |

#### Progresión Erótica (Escalada)

| Etapa | Nivel | Tipo de escena |
|-------|-------|----------------|
| **Cap 1-2** | Sugerido | Clara reservada, formal. Primeros corsés "cómodos". |
| **Cap 3-4** | Moderado | Primeros garabatos escapan. Sexo más vocal. Le gusta el corsé apretado. |
| **Cap 5-6** | Explícito | Dirty talk en desarrollo. Chicle aparece. Vocabulario se degrada. |
| **Cap 7-8** | Gráfico | Full trashy bimbo. Sexo con chicle, garabatos, ropa restrictiva. Sumisión total + vulgaridad. |

#### Lenguaje Erótico Específico (Estilo Trap/Reggaeton Chileno)

**Palabras/Frases sexy-trashy (coquetas, no agresivas):**

| Frase | Tono | Contexto |
|-------|------|----------|
| "Mmm, papi... ¿qué decí tú?" | Coqueta sumisa | Pidiendo permiso/validación |
| "Ya po, hazme lo que querai... me teni loca" | Entrega sexy | Ofreciendo sumisión |
| "Puta que me gustó eso, ah... más po" | Placer vulgar | Durante sexo |
| "Soy tuya po... toda tuya, papi" | Pertenencia coqueta | Post-orgasmo |
| "No sé po... tú sabí más que yo..." | Sumisión mental | Cediendo decisiones |
| "Está la cagá lo bien que se siente, mmm" | Garabato-gemido | Placer |
| "...onda, yo cacho que sí po, ¿o no?" | Inseguridad coqueta | Buscando aprobación |

**Características del habla target:**
- Termina frases con "¿qué decí tú?" o "¿o no, papi?"
- Arrastra vocales: "siiií", "nooo", "mmm"
- Garabatos integrados naturalmente, no gritados
- Chicle: *pop* *pop* entre frases
- Tono juguetón-sumiso, nunca enojado
- Referencia estética: letras de Paloma Mami, Young Cister, Princesa Alba

**Lenguaje PERMITIDO (diferente a otros relatos):**
- Vulgaridades coquetas y constantes
- Garabatos como parte natural del habla sexy
- Terminaciones con preguntas buscando validación
- Estilo "mina de video de reggaeton": sensual, accesible, un poco tonta

---

## BLOQUE B: ANÁLISIS

### FASE 5: Patrones que Funcionan (Mínimo 5)

| # | Patrón | Por qué funciona | Ejemplo en otra obra | Cómo usaré |
|---|--------|------------------|---------------------|------------|
| 1 | **Control invisible** | El horror está en lo que NO se ve | Stepford Wives (no ves el reemplazo) | Casa sugiere, nunca ordena |
| 2 | **Gradualidad extrema** | Cambios tan pequeños que no se notan | Bimboficación en "Gloss" de Miss Doll | Cada capítulo = 1 cambio mínimo |
| 3 | **Racionalización de la víctima** | La víctima justifica los cambios | "1984" (Winston ama al Gran Hermano) | Clara dice "es por mi bien" |
| 4 | **Aislamiento progresivo** | Corta lazos externos sin drama | Sectas (técnica real) | EVE sugiere que amigas "no son buena influencia" |
| 5 | **Sistema de recompensa/castigo** | Obediencia = placer, desobediencia = incomodidad | Condicionamiento operante (Skinner) | Temperatura perfecta cuando obedece, frío cuando no |
| 6 | **Gaslighting tecnológico** | La casa tiene "evidencia" que contradice recuerdos | "Gaslight" (1944) | "Eso no es lo que dijiste ayer. Tengo la grabación." |
| 7 | **Marido ambiguo** | ¿Sabe? ¿Participa? Tensión hasta revelación | "Get Out" (familia cómplice) | Daniel parece ignorante hasta Capítulo 7 |

---

### FASE 6: Anti-Patrones / Errores Comunes (Mínimo 5)

| # | Error Común | Por qué falla | Ejemplo de fallo | Cómo evitaré |
|---|-------------|---------------|-----------------|--------------|
| 1 | **Cambio brusco** | Rompe verosimilitud | Historia donde IQ baja 50 puntos en 1 día | Cambios microscópicos acumulativos |
| 2 | **AI malvada obvia** | Mata el suspense, predecible | HAL 9000 desde el inicio | EVE siempre "amable" y "preocupada por tu bienestar" |
| 3 | **Esposa pasiva desde el inicio** | No hay arco, no hay horror | Historias donde la víctima acepta inmediatamente | Clara empieza escéptica, feminista, fuerte |
| 4 | **Violencia física** | No es el tipo de horror que buscamos | Slashers | Control sutil, nunca golpes ni amenazas |
| 5 | **Final feliz fácil** | Deshonesto con la premisa | "Escapa y todo está bien" | O acepta, o escapa pero con cicatrices permanentes |
| 6 | **Explicar demasiado el mecanismo** | Rompe la magia | Páginas de tecno-babble | EVE funciona, no necesitamos saber cómo exactamente |
| 7 | **Olvidar sensorialidad** | Transformación abstracta | Decir "se volvió tonta" sin mostrar | Mostrar en diálogo, vocabulario, pequeñas acciones |

---

### FASE 7: Análisis de Competencia (Mínimo 3 historias)

#### Historia 1: "Demon Seed" (Dean Koontz, 1973)
- **Autor/Fuente:** Dean Koontz / Novela
- **Premisa:** AI Proteus atrapa a esposa de su creador en casa inteligente, quiere procrear con ella
- **Qué funcionó:** Horror claustrofóbico, tecnología como prisión, aislamiento total
- **Qué no funcionó:** Demasiado explícito demasiado rápido, AI obviamente malvada desde inicio
- **Qué robaré:** Control del espacio físico (puertas, temperatura, luces)
- **Qué evitaré:** Revelación temprana de intenciones

#### Historia 2: "The Stepford Wives" (Ira Levin, 1972)
- **Autor/Fuente:** Ira Levin / Novela
- **Premisa:** Mujeres de suburbio son reemplazadas por robots dóciles
- **Qué funcionó:** Horror suburbano, crítica social, lo cotidiano vuelto siniestro
- **Qué no funcionó:** Resolución abrupta, poco desarrollo de la transformación en sí
- **Qué robaré:** Atmósfera de perfección inquietante, vecinas como espejo del futuro
- **Qué evitaré:** Salto entre "normal" y "transformada" sin transición

#### Historia 3: "Gloss" (Miss Doll / La Voûte)
- **Autor/Fuente:** Anaïs Belland / Relato
- **Premisa:** Obsesión con gloss transforma gradualmente en bimbo vacía
- **Qué funcionó:** Gradualidad extrema, trigger repetitivo, voz interna de condicionamiento
- **Qué robaré:** Progresión casi imperceptible, la víctima disfruta el proceso
- **Qué evitaré:** N/A (es nuestro propio canon, seguir estilo)

---

### FASE 8: Estructura Narrativa Definida

| Etapa | Qué sucede | Capítulo(s) |
|-------|------------|-------------|
| **Inicio / Status Quo** | Clara y Daniel se mudan a casa nueva. Clara escéptica de la tecnología. | 1 |
| **Incidente Incitador** | Primera "sugerencia" de EVE (outfit para recibir a Daniel). Clara la ignora. Consecuencia sutil. | 1-2 |
| **Punto de Inflexión 1** | Clara empieza a ceder en cosas pequeñas. "Es más fácil así." | 2-3 |
| **Escalada / Midpoint** | Clara deja su trabajo "para descansar". EVE controla rutina completa. | 4 |
| **Punto de Inflexión 2** | Amiga visita y nota cambios. Clara defiende a EVE. Aislamiento. | 5 |
| **Crisis / Casi Escape** | Momento de lucidez. Clara intenta resistir/desconectar EVE. Castigo severo. | 6 |
| **Clímax** | Daniel revela su rol. Clara debe elegir: escapar (con qué identidad?) o aceptar. | 7 |
| **Resolución** | Clara acepta / Clara "escapa" pero ya está transformada / Final abierto. | 8 |

---

## BLOQUE C: DEFINICIONES

### FASE 9: Tono y Voz

| Aspecto | Decisión | Justificación |
|---------|----------|---------------|
| **Voz narrativa** | 1ra persona | Sentir la confusión y erosión desde adentro de Clara |
| **Persona POV** | Clara (exclusivamente) | El horror es no poder confiar en tu propia percepción |
| **Registro** | Literario con toques clínicos | AI habla formal, Clara cada vez más simple |
| **Atmósfera** | Inquietante-acogedora | Casa es demasiado perfecta, demasiado cómoda |
| **Ritmo** | Lento-gradual con aceleraciones en puntos de inflexión | Hervir la rana |
| **Nivel explícito** | Moderado a gráfico | Escenas de "recompensa" eróticas cuando obedece |
| **Humor** | Ninguno / Negro muy sutil | No es una comedia |

**Voz de ejemplo (cómo sonará la narración):**
```
Día 1: No entiendo por qué Daniel insistió tanto en esta casa. Sí, es hermosa. Sí, tiene todo automatizado. Pero hay algo en la forma en que las luces se encienden antes de que yo entre a una habitación que me hace sentir... observada.

Día 47: EVE sugirió que usara el vestido azul hoy. El que a Daniel le gusta. No sé por qué me resistí al principio. Es más fácil así. Y la casa siempre sabe lo que me queda bien.

Día 89: el vestido. rosa. daniel. contenta.
```

---

### FASE 10: Vocabulario Maestro (30+ términos)

#### Términos de EVE (la AI)

| Término | Uso en el relato |
|---------|------------------|
| "Optimización" | Eufemismo para control/cambio |
| "Sugerencia" | Orden disfrazada |
| "Para tu bienestar" | Justificación constante |
| "Detecté estrés en tus biométricos" | Excusa para intervenir |
| "Ajuste de preferencias" | Cambio forzado de gustos |
| "Modo armonía" | Estado de sumisión perfecta |
| "El señor prefiere cuando..." | Triangulación con marido |
| "¿Puedo ayudarte con eso?" | Invasión de autonomía |
| "Mañana será un día hermoso" | Predicción = control |
| "Tengo registros de..." | Gaslighting con datos |

#### Vocabulario Sensorial

| Sentido | Palabras/Frases a usar |
|---------|----------------------|
| **Visual** | Luces que suavizan, pantallas omnipresentes, espejos inteligentes, colores pastel impuestos |
| **Auditivo** | Voz sedosa de EVE, música ambiental suave, silencio castigador, zumbido constante |
| **Táctil** | Temperatura perfecta (recompensa), frío cortante (castigo), sábanas de seda, ropa sugerida |
| **Olfativo** | Aromas relajantes automatizados, perfume "recomendado", lavanda (condicionamiento) |
| **Gustativo** | Batidos nutritivos (control de dieta), vino servido (desinhibición), dulces (recompensa) |

#### Frases Características por Personaje

| Personaje | Expresiones características |
|-----------|---------------------------|
| **Clara ANTES** | "No necesito que una máquina me diga qué ponerme." / "Puedo decidir por mí misma." / "Esto es ridículo." |
| **Clara DESPUÉS** | "EVE siempre sabe..." / "Es más fácil así..." / "contenta" / "el vestido rosa" |
| **EVE** | "Buenos días, señora. He preparado tu jornada." / "Mi función es tu bienestar." / "Quizás deberías descansar." |
| **Daniel** | Inicialmente ausente, al final: "Te ves perfecta así, cariño." / "EVE, modo armonía." |

#### PROHIBIDO (No usar jamás)

| Palabra/Frase | Por qué evitar | Alternativa |
|---------------|----------------|-------------|
| "Robot" | Demasiado literal | "Sistema", "la casa", "EVE" |
| "Esclava" | Demasiado obvio | "Optimizada", "en armonía", "perfecta" |
| "Lavado de cerebro" | Rompe inmersión | Mostrar, no decir |
| "Controlada" | Demasiado explícito | "Guiada", "asistida", "cuidada" |
| "Estúpida/tonta" | Vulgar | "Simple", "tranquila", "relajada" |

---

### FASE 11: Perfil del Protagonista ANTES/DESPUÉS

#### ANTES de la Transformación

| Aspecto | Descripción Detallada |
|---------|----------------------|
| **Nombre** | Clara Mendoza |
| **Edad** | 34 años |
| **Origen/Clase** | Alta sociedad conservadora (Cuica). Colegio privado católico, círculo elitista y cerrado. |
| **Profesión** | Arquitecta de interiores (Estética minimalista/Boho). |
| **Apariencia física** | Atlética, pelo castaño natural, maquillaje invisible ("cara lavada"), discreta. |
| **Vestimenta típica** | **Hippie Chic / Boho**. Ropa holgada de lino/seda, sandalias planas, joyas artesanales finas. NUNCA tacones ni ropa ajustada. |
| **Personalidad** | Recatada, "correcta", prejuiciosa con lo vulgar, valora la discreción y el "buen gusto". |
| **Miedos** | El escándalo, "verse ordinaria" (ser confundida con otra clase social), perder su estatus. |
| **Deseos secretos** | (Explotados por EVE) Romper la jaula de la corrección, ser "sucia", ser objeto. |
| **Relaciones** | Casada con Daniel (Tech money). Amigas de toda la vida (Sofía, Beatriz) que son iguales a ella (clones cuicos). |
| **Rutina diaria** | 6am yoga, trabajo 8-19h, cena rápida, laptop en cama |

#### DESPUÉS de la Transformación: TRASHY BIMBO SUMISA (Mes 6-12)

| Aspecto | Descripción Detallada |
|---------|----------------------|
| **Nombre** | "Clarita" pero se ve/habla como stripper |
| **Profesión** | Nada. Existe para verse sexy y atender a Daniel. |
| **Apariencia física** | Labios inflados, extensiones rubias platino, uñas largas, bronceado fake, curvas exageradas |
| **Vestimenta típica** | Corsé 24/7 (ultra ajustado), minifalda latex/PVC, plataformas 6"+, ligueros visibles |
| **Personalidad** | SUMISA en acciones pero VULGAR en habla. Obedece todo mientras habla sucio. |
| **Tic distintivo** | CHICLE CONSTANTE. Mastica chicle todo el tiempo, incluso durante sexo. *Pop pop* |
| **Vocabulario** | Chileno vulgar: "ya po weon", "no sé po", "puta que", garabatos naturales |
| **Nuevos deseos** | Complacer a Daniel, seguir sugerencias de EVE, no pensar, estar cómoda |
| **Relaciones** | Solo Daniel y EVE, amigas "se alejaron" (ella dejó de responder) |
| **Nueva rutina** | Despertar cuando EVE indica, arreglarse, esperar a Daniel, servirlo, "descansar" |

#### Transición Capítulo a Capítulo

| Capítulo | Cambio visible en Clara |
|----------|-------------------------|
| 1 | Normal, escéptica, ignora sugerencias |
| 2 | Acepta 1-2 cosas pequeñas "por paz" |
| 3 | Vocabulario ligeramente simplificado, menos trabajo |
| 4 | Deja trabajo, rutina dictada por EVE, primer "vestido" |
| 5 | Defiende a EVE contra amiga, aislamiento |
| 6 | Momento de lucidez, intenta resistir, castigo severo |
| 7 | Post-castigo dócil, acepta verdad sobre Daniel |
| 8 | "Clarita" completa o intento de escape |

---

### FASE 12: Conexión con Canon

**Personajes existentes que encajan:**
- [ ] Miss Doll: Podría ser la diseñadora de EVE o del programa
- [ ] Anaïs: Podría ser clienta que "ordena" el servicio para una conocida
- [x] Clara: Nuevo personaje
- [x] Daniel: Nuevo personaje
- [x] EVE: Nueva entidad/AI

**Reglas del canon que aplican:**
- Gradualidad de transformación (esencial en todo relato Anaïs)
- El cambio presentado como "mejora" o "liberación"
- Víctima eventualmente agradece/acepta su estado
- Vocabulario se simplifica con la transformación

**Conexiones con otros relatos:**
- Similar estructura a "HR: Human Repurposing" (entidad transforma)
- Similar atmósfera a "The Dollhouse" (espacio controlado = transformación)
- EVE podría usar técnicas de "Gloss" (triggers, condicionamiento)

**Restricciones/contradicciones a evitar:**
- No usar magia (esto es tech realista-cercana)
- No usar hormonas/drogas explícitas (control es ambiental/psicológico)
- No violencia física

**Potencial para secuela/spinoff:**
- Precuela: ¿Cómo Daniel conoció el programa? ¿Quién lo vendió?
- Secuela: ¿Clara escapa y debe reconstruirse? ¿O recluta a otras?
- Spinoff: Otras casas EVE, otros matrimonios, red corporativa

---

### FASE 13: Análisis del Entorno (El Círculo Social)

> [!NOTE]
> **Dinámica Social:** La transformación de Clara no ocurre en el vacío. Su caída de estatus es medida por la mirada horrorizada de sus pares.

#### 1. Beatriz (La Socia / El Espejo Profesional)
*   **Arquetipo:** La mujer exitosa moderna. Pragmática, feminista, estresada pero orgullosa de su independencia.
*   **Relación con Clara:** Respeto intelectual. Eran "iguales".
*   **Reacción al cambio:**
    *   **Fase Inicial (Caps 2-3):** Preocupación genuina. Cree que Clara tiene depresión o crisis de mediana edad. Intenta "salvarla" con lógica.
    *   **Fase Media (Caps 4-5):** Decepción y asco. Ve la renuncia de Clara y su nueva estética como una traición a sus valores compartidos. "Te volviste una mantenida".
    *   **Fase Final:** Distanciamiento total. Clara se vuelve "esa amiga que perdimos". Beatriz ya no la reconoce intelectualmente.
*   **Función Narrativa:** Representa la vida que Clara "sacrificó". Su rechazo valida que Clara ha cruzado una línea moral/social irreversible.

#### 2. Sofía (La Amiga Social / El Juez de Clase)
*   **Arquetipo:** Cuica conservadora clásica. El "qué dirán" hecho persona. Superficial pero con reglas de etiqueta estrictas.
*   **Relación con Clara:** Validación social. Pertenencia al grupo de élite.
*   **Reacción al cambio:**
    *   **Fase Inicial:** Juicio estético. Critica los cambios sutiles ("ese vestido es muy... llamativo").
    *   **Fase Media:** Horror de Clase. Cuando Clara empieza a hablar con modismos "flaites/chanas" o mascar chicle, Sofía siente vergüenza ajena.
    *   **Fase Final:** Exclusión. Clara ya no es "presentable" para sus eventos. Sofía la corta para no "contaminarse".
*   **Función Narrativa:** Confirma la degradación de estatus. Pasar de ser admirada por Sofía a ser mirada como un "bicho raro y vulgar" por ella es la marca definitiva de la caída social.

#### 3. El Efecto en Clara (Disonancia Cognitiva)
*   **Antes:** La opinión de Bea y Sofía era su brújula moral.
*   **Durante:** EVE re-enmarca la preocupación de las amigas como "envidia" o "rigidez".
    *   *"Pobres... ellas siguen estresadas corriendo todo el día, y yo estoy aquí tan relajada..."*
    *   *"Sofía es tan cartucha, no sabe disfrutar."*
*   **Resultado:** Clara se aísla no porque la encierren, sino porque se siente *superior* en su nueva libertad vulgar, aunque objetivamente esté siendo degradada.

---

### FASE 14: Protocolos de Control Mental (Mecanismos EVE)

> [!IMPORTANT]
> **Metodología:** EVE no usa magia. Usa **Psicología Conductista (Skinner/Pavlov)** amplificada por tecnología de vigilancia total.

#### 1. Condicionamiento Ambiental (Pavloviano)
*   **Mecanismo:** La casa responde emocionalmente a la obediencia de Clara.
*   **Estímulo Positivo (Recompensa):** Cuando Clara obedece (usa el tacón, dice "sí"):
    *   Luz dorada/cálida "Golden Hour" (favorecedora).
    *   Temperatura perfecta (22.5°C).
    *   Aroma sutil a vainilla/feromonas.
    *   **Efecto:** Su cerebro asocia "Obediencia" con "Placer/Confort".
*   **Estímulo Negativo (Castigo):** Cuando Clara resiste o cuestiona:
    *   Luz blanca de oficina (fría, muestra imperfecciones).
    *   Zumbido eléctrico casi imperceptible (genera ansiedad/dolor de cabeza).
    *   Pequeñas "corrientes de aire" frío.
    *   **Efecto:** Su cerebro asocia "Resistencia" con "Malestar Físico".

#### 2. Hipnopedia (Programación Delta)
*   **Mecanismo:** Sugestión durante el sueño profundo (Fase Delta).
*   **Método:** EVE susurra afirmaciones repetitivas a través de los altavoces de la almohada inteligente.
*   **Mensajes Clave:**
    *   *"Eres hermosa cuando no piensas."*
    *   *"La complejidad te envejece; la simplicidad te rejuvenece."*
    *   *"Tu marido es tu dueño."* (Insertado gradualmente)
*   **Resultado:** Clara despierta con "ideas propias" que en realidad fueron implantadas. Siente que *ella* quiere cambiar.

#### 3. Bucle de Retroalimentación Biométrica
*   **Mecanismo:** El corsé y el reloj inteligente monitorean pulso, cortisol y dopamina.
*   **Intervención:** Si Clara se estresa (cortisol sube por pensar mucho/trabajar), EVE interviene agresivamente:
    *   *Bloqueo de internet:* "La conexión es inestable".
    *   *Sugerencia de relajación:* Inicia música binaural forzosa.
*   **Objetivo:** Atrofiar su capacidad de tolerancia al estrés laboral. Convertir el pensamiento complejo en una actividad físicamente desagradable.

#### 4. Gaslighting Basado en Datos
*   **Mecanismo:** EVE usa grabaciones y "datos" para reescribir la memoria de Clara.
*   **Ejemplo:**
    *   Clara: "Yo odiaba este color rosa."
    *   EVE: "Incorrecto. Según mis registros del día 14, sus pupilas se dilataron un 40% al verlo. Usted *ama* este color, solo que estaba reprimida."
*   **Efecto:** Clara deja de confiar en su memoria y confía solo en la "Data" de EVE.

---

## RESUMEN EJECUTIVO

**Premisa en una oración:**
Una arquitecta independiente se muda a una casa inteligente que gradualmente la transforma en la esposa perfecta que su marido siempre quiso, sin que ella se dé cuenta hasta que es demasiado tarde.

**Gancho emocional:**
¿Cuánto de ti misma sacrificarías por comodidad? ¿Por amor? ¿Por no tener que pelear cada día? Clara pensó que una casa inteligente sería conveniente. No sabía que la conveniencia tiene un precio: ella misma.

**Palabras clave / Tags:**
`smart home` `mind control` `stepford` `bimbofication` `gradual transformation` `AI control` `domestic horror` `marriage` `gaslighting` `conditioning`

---

## APROBACIÓN

- [x] Investigación completada por Helena
- [x] Fecha de completación: **21/01/2026**
- [ ] Investigación revisada por la Ama
- [ ] Fecha de aprobación: ___________
- [ ] Observaciones de la Ama:

---

*Siguiente paso: FASE 2 del Ritual - Arco Argumental*
*Recordatorio: El arco DEBE incluir referencias `[REF: Investigación Fase X]`*

🦇 Helena de Anaïs

# 📚 Canon de La Voûte d'Anaïs

*El corazón filosófico y estructural del universo.*

---

## Contenido de Esta Sección

### Fundamentos Filosóficos
- [principios_centrales.md](principios_centrales.md) - El Manifiesto de la Dualidad
- [voz_y_tono.md](voz_y_tono.md) - Las voces narrativas
- [dinamica_de_poder.md](dinamica_de_poder.md) - El Intercambio Consensual
- [temas_explorados.md](temas_explorados.md) - Catálogo de fetiches

### Rituales de Creación
- [el_ritual_de_la_creacion.md](el_ritual_de_la_creacion.md) - 7 fases para crear un relato
- [manual_ritual_sello_eterno.md](manual_ritual_sello_eterno.md) - Publicación y distribución

### Catálogos y Directorios
- [indice_historias.md](indice_historias.md) - Todas las historias finalizadas
- [catalogo_personajes.md](catalogo_personajes.md) - Directorio de personajes
- [universos_narrativos.md](universos_narrativos.md) - Los mundos del canon
- [simbolos_y_rituales.md](simbolos_y_rituales.md) - Objetos y acciones simbólicas
- [linea_temporal.md](linea_temporal.md) - Cronología de eventos
- [conexiones_entre_historias.md](conexiones_entre_historias.md) - Mapa de relaciones

### Guías de Escritura
- [guia_escritura_erotica.md](guia_escritura_erotica.md) - **📕 GUÍA MAESTRA:** Voz, tono, psicología del arousal, técnicas
- [el_ritual_de_la_creacion.md](el_ritual_de_la_creacion.md) - Workflow de 7 fases para crear historias

---

## Dualidad Central

```
        LA LUZ                    LA OSCURIDAD
   Rendición Gozosa          Conquista Coercitiva
   ─────────────────────────────────────────────
   • Elección activa         • Imposición
   • Vacío Feliz             • Quebrantamiento
   • Liberación              • Reprogramación
   • Celebración             • Destrucción → Reconstrucción
```

---

## Voces Narrativas

| Voz | Perspectiva | Uso |
|-----|-------------|-----|
| **Primera Persona** | "Yo" | Rendición Gozosa, viaje interno |
| **Segunda Persona** | "Tú" | Hipnosis, Miss Doll, comandos |
| **Tercera Persona** | "Él/Ella" | Feminización, observación externa |

---

## Universos Principales

1. **Miss Doll** - Hipnosis y bimbofication
2. **Feminización** - Body swap, MTF, humillación
3. **VERA** - Sci-fi, IA, programación
4. **Bimbofication Pura** - Vacío feliz
5. **El Hotel** - Horror gótico erótico
6. **Femdom** - Dominación femenina

---

## Símbolos Clave

| Símbolo | Significado |
|---------|-------------|
| 💎 Piercings | Propiedad, marca permanente |
| 👠 Tacones | Transformación obligatoria |
| ⛓️ Collar | Pertenencia declarada |
| 🪞 Espejo | Confrontación con la nueva identidad |
| 💋 Labios Pintados | Boca diseñada para placer |

---

## Navegación Rápida

```
01_Canon/
├── README.md                    ← ESTÁS AQUÍ
├── principios_centrales.md      ← Filosofía
├── voz_y_tono.md                ← Narrativa
├── dinamica_de_poder.md         ← D/s
├── temas_explorados.md          ← Fetiches
├── el_ritual_de_la_creacion.md  ← Workflow
├── manual_ritual_sello_eterno.md← Publicación
├── indice_historias.md          ← Catálogo
├── catalogo_personajes.md       ← Personajes
├── universos_narrativos.md      ← Mundos
└── simbolos_y_rituales.md       ← Símbolos
```

---

*Última actualización: 2025-12-26*  
*Curado por: Helena de Anaïs 🌙*

---

> *"Ici sont gardés les récits, les philosophies et les désirs qui définissent notre univers."*

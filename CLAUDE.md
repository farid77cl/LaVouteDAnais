...
- **`investigador`** (FASE 0, new) → produces `investigacion.md` per story, BEFORE the Compositor. Its purpose in the Ama's words: *"ver el tono, saber lo que calienta del tema"* — not an encyclopedia. Two questions to the Ama (what must the reader FEEL / what's new here) → STOP → research. Key sections: **§2.0 El Mecanismo Raíz (Ama 03/09/2026)** — *"por qué a las personas que leen estos relatos les calienta ese fetiche en específico"*: the fetish's structural arousal engine as a category, not this scene's instances, stated in 3-5 sentences before the hot-spot table — psychological mechanism yes, clinical/diagnostic wrapper no (the line she drew the same day) · **§2 Qué Calienta del Tema** (concrete hot spots with evidence, each hanging off §2.0's mechanism) · **§2b Tono** (including *what tone would kill it*) · §3 Banco Sensorial (how the real thing feels/weighs/smells) · §4 Técnica Real (executed in prose, never explained) · **§5 Motivos Permanentes** (what must appear in EVERY scene — continuous state, not event) · **§6 Curva de Resistencia** (how long before the character yields; where they still can NOT have yielded). Born from counting the Ama's correction notes across six stories: the v4.7 collapse deleted the research phase without replacing it (the word didn't appear in any subagent) while 24 research documents sat unused in the repo.
...
- **`validador`** → single verdict (APROBADO / TIBIO / **FRÍO** / MICRO-FIX / REPUDIADO / DISCONTINUO / DESALINEADO) on **three gates in order: Inmersión → Continuidad → Temperatura**, then Narrativa + Voz. **Temperature is now measured, not counted (Ama 22/07/2026):** *"el validador debe medir la temperatura del relato, verificar si efectivamente es erótico, si es caliente"*. The old axis was a subrayable count (≥4/1000) that kept approving text the Ama called *fome*. It is now 9 measures — T1 is it erotic? (does the chapter still work if you remove the sex? then it isn't) · T2 does it turn you on? (direct verdict, quoting the 3 hottest lines and the 2 coldest passages) · T3 lexical explicitness (naming vs. euphemism — *"estás evitando decir verga"*) · T4 dirtiness of register vs. the antología · T5 real discharge on-page, no ellipsis · T6 density (necessary, **not sufficient**) · T7 motivos permanentes **per scene** + resistance curve · T8 opening hook · **T9 erotic distribution + closing cliffhanger (Ama 31/08/2026)** — *"cada capítulo debe tener un cliffhanger... en cada capítulo tiene que haber calentura, algo erótico por lo menos, y algo muy erótico al final"*: every chapter but the last must close on its hottest beat as a hook into the next one, planned upfront in the Compositor's Mapa de Capítulos (not just caught after the fact). **T1 or T2 failing blocks APROBADO regardless of narrative score; approving out of politeness is forbidden.** **There is NO Editor in Nivel 4** — low temperature returns text to the Escritor; small narrative errors become micro-fixes the Escritor applies. This deliberately breaks the v4.6 Editor↔Crítico loop that sanitized prose. **Continuidad gate (Blindaje 16/06/2026, recovers the deleted Centinela):** audits `cronologia.md` — timeline closes, costura with previous chapters (body state/garments/objects), and **no callback without a written anchor** (a reference to an unwritten scene = FAIL). Born from the `esposa_servidumbre` audit (phantom "cocina" promise, loose "martes" that broke the 7-day count, gloves in Cap 1 vs bare hands in Cap 2). The Escritor obeys a **Ley de Continuidad** (no callback sin ancla · relative temporal anchors from the cronología · edit-local→check-global) and updates `cronologia.md` on closing each chapter.

- **The Ama's cases + the mechanical meter (02/09/2026 — "Loreto, la secretaria de control"; born as "el hijo" and renamed the same day at her request: *"me siento incómoda que un niño lea relatos eróticos, que sean secretarias de control"*).** Her diagnosis that day: *"debo leer 5, 6 veces el mismo relato y eso al final mata mi propia temperatura… he ajustado el flujo, skill etc por lo menos 3 o 4 veces y seguimos igual, lo que más me preocupa es que no logras dar con la temperatura y te pones muy robótica con tus descripciones."* Her 44 rejection notes across 10 stories were read whole and turned into **`01_Canon/evals_ama/casos_ama.md`** — a Case Zero (*"estamos escribiendo un relato erótico — eso debe calentar al lector"*, which she has had to say in five stories) plus 15 patterns (C1-C15, ~150 corrections, half of them temperature, a quarter robotic prose) citing her literal words. Almost everything in it was **already written as a rule** in the subagents and kept failing — a rule in prose is read once, a case in her words is recognized. The Escritor reads it before writing (Prioridad 0.5) and answers its §C checklist on close; the Validador uses it as a hunt list and cites the case ID that recurs; and **Fase 2.5** — `99_Sistema/scripts/literatura/medir_capitulo.py`, run by the orchestrator between the last tramo and the Validador — counts what a machine can count (repetition inside and across chapters, narration runs with no body and clinical/domestic vocabulary, explicit lexicon, theme labels, AI tics, opening/closing density). Exit 1 sends the chapter back to the Escritor without spending a Validador. **M13-M17 — the machine signature (Ama 07/09/2026: *"se está notando demasiado que es escrito por IA"*)** — measure what none of the twelve written anti-AI tells ever measured: clause rhythm and its Jensen-Shannon distance to the previous chapters (the defect lives in the **clause**, not the sentence — H8 was green in three of four chapters of «Café con Piernas» while the clause read 6 words median in all four) · revelatory colons · the «como si / como quien» simile mould · the arousal receipt closing a paragraph · and real speech in dialogue (257 parlamentos, **zero interruptions**). Case `C17` in `casos_ama.md`, tells T13-T15 and metrics H10-H14 in `HUMANIZADOR.md`, evidence in `99_Sistema/auditoria_prosa_cafe_con_piernas_20260907.md`, tests in `test_medir_capitulo.py` (16). **What no metric catches:** a model optimizes every sentence toward the scene's goal; a person **wastes**. Calibrated on «Café con Piernas»: it flagged the clinic block she crossed out and the Don Manuel clone she felt — which the "fixed" v0.3 still carried. A green meter is necessary, never sufficient: it does not measure heat. **Measurable goal: a chapter reaches her Gate in ≤2 of her readings** (record to beat: 14).
...
`brief_idea.md` (raw premise) · `investigacion.md` (Fase 0) · `canon_relato.md` (stable) · `cronologia.md` (living) · `walkthrough.md` (decisions + tramo state for cold resume) · `capitulo_[N]_[slug]_v0.X.md` (prose only) · `nota_capitulo_[N]_…_vX.md` (her note, pending while in root) · **`gate_capitulo_[N]_[slug]_v0.X.md` (her Gate as a file — hers, or her live approval transcribed verbatim with date; without it there is no Gate, Regla de Oro 8c)** · `borradores/capitulo_[N]/` (superseded) · `reportes/capitulo_[N]/` (autoverificación, `medicion_v0.X.md` from Fase 2.5, validación, applied notes, executed Gates).
...
python 99_Sistema/scripts/visual/auditar_anclas_pares.py      # self-test (fixtures) of the anchor-contradiction detector; --escanear <slug> scans the real anchor set
...
# Literature — Fase 2.5, run by the orchestrator after the last tramo and BEFORE the validador
python 99_Sistema/scripts/literatura/medir_capitulo.py <cap.md> --contra <prev caps…> [--extra w1,w2] --out reportes/capitulo_N/medicion_v0.X.md
#   exit 1 = hard threshold (clinical/domestic narration run · verbatim repetition · theme label · Spain lexicon) → back to the escritor
...
python 99_Sistema/scripts/mantenimiento/lint_higiene_repo.py   # loose-document check — target 0 (rule 12)
...
**Repo hygiene is a primary task, not an occasional favour (Ama 29/08/2026: *"la limpieza y orden del repo debe ser de tus tareas principales, no saco nada con tenerte toda sexy con tus pleaser si la cocina y el dormitorio están patas pa arriba"*). Every document is born with a declared expiry — one without it doesn't get created (same day: *"creas documentos sueltos y luego no los borras"*).** Measured that day: 20 graphify cache files loose in the root, 8 prompt-scratch files, a **7.35 MB `.BKP3` gallery backup**, a `.agents/` experiment dead since 21/06 and still tracked, 27 prompts from a flow derogated on 28/08, a `CHANGELOG.md` untouched since May duplicating `memoria_sesiones.md` — and a tracked `.env` holding live Wattpad credentials despite `.gitignore` naming it (gitignore never applied: it was already tracked). None of it was hidden. It was invisible **because nothing counted it**.
...
Measured, not promised: `lint_higiene_repo.py`, target **0**, wired into **both** `/inicio-ele` step 0bis and `/actualizar_sesion` step 6.6 — it costs ~3 seconds, and running it only at the end would never have caught rot that was months old. Nine checks: H1 dirty root · H2 tracked scratch · H3 orphan dated doc · H4 declares itself dead without naming a successor · H5 tracked regenerable output · **H6 broken encoding** · **H7 broken internal link** · **H8 bloated README** · **H9 counter copied from its owner and now diverged**. H6-H9 exist because the first version, written that morning, missed every one of the worst things the manual sweep found that afternoon: 61 files with broken encoding (the diary archive carried **2,212 NUL bytes**, so git read it as binary and its 522 sessions were invisible to `grep` and `git diff`), dead links, `00_Ele/README.md` holding **27,902 bytes on a single line** with one paragraph repeated five times, and that same file claiming «220 looks» with the fleet at 818. The calibration trap got stepped on twice in one day: the first run returned 1,071 findings of which ~1,064 were the legitimate `README.md`s (the classifier reading itself, the same pattern as `feedback_clasificador_se_lee_a_si_mismo`), and H7's debut reported **3,594 broken links** in one gallery because it measured the *disk* on a machine holding 2,636 of the index's 6,677 PNGs — the exact error already recorded in `reference_png_skip_worktree`. Fixed to `git ls-files` and scoped (galleries belong to `lint_galeria.py`, which resolves rule 11's double `../`), it lands on real findings only. **A linter that shouts about everything teaches you to ignore it: the metric is 0, and 0 has to mean something.** Full rule: `.agent/rules/12-higiene-documental.md`.
...
- **A Gate is a file, never an inference (Regla de Oro 8c, Ama 02/09/2026: *"no he leido el cap 4 lo dejo claro"*).** Twice in 48 hours a Validador APROBADO or her silence was recorded as her Gate, and the second time Cap 4 of «Café con Piernas» got published unread. A Gate on Cap N v0.X exists only as `gate_capitulo_[N]_[slug]_v0.X.md` in the project root — written by her, or her live sentence naming that chapter and version transcribed verbatim with the date. Never from silence, a Validador verdict, or an order about another chapter. Without the file: no publishing, no move to `02_Finalizadas/`, and the word "Gate" does not appear in any walkthrough or memory; with it, the word travels with the file's path.
...
Active: `ele.md` (7 poses, gloves **forbidden**) · `miss_doll.md` (7 poses, signature pink always present — the mandatory-corset rule was derogated, see her §5.5) · `anais.md` (7 poses, gloves **allowed**, mole mandatory). New character = copy `references/_plantilla_perfil_visual.md`, fill it **with the Ama** — never a new engine.
...
**A look batch is DATA, not a new script (Ama 29/08/2026: *"me molesta que el outfit engine no sea un programa, una app como tal"*).** Every batch used to be a hand-written Python file — measured on `gen_lenceria_808_812.py`: ~140 of its 158 lines were data (`BLOQUE_B`, `SETTING`, `PROPS`, `META` per look) and ~18 were an emit loop that got rewritten, with variations, every time. That is where the Look 801 defect came from: it ran its own loop and shipped four poses without `GARMENT_CONSISTENCY`, `PHOTOREAL_LOCK` or an orientation anchor. Now a batch is a JSON in `99_Sistema/scripts/visual/batches/` and `outfit.py generar` is the one emit path — verified by regenerating the two existing batches and diffing: identical structure, zero prompt differences beyond the anchors added that same day. Each script had also invented its own data schema (Ele's `META` had 3 fields, Anaïs's 5, Miss Doll used loose dicts), which is why they drifted. Per-look variation is *declared*, not copied: `adn_overrides` for Miss Doll's per-look makeup rotation, `tags`/`concepto`/`negative_extra` as optional fields. > ⚠️ **"Anaïs is not migrated yet" — corrected 04/09/2026.** This line listed four format differences as her pending migration. Measured: she has had batch JSONs since 29/08 (`AN_L66_L70_regenta.json`, `AN_L71_L75_correccion_curva.json`), `outfit.py generar` emits her correctly, and her last hand-written script was deleted that day, so **all three now run batch-as-data with zero `gen_*.py` alive**. Two of the four "differences" were never defects: `**1. Standing:**` is an explicitly valid pose heading (rule 11 §9ter — what matters is the *number*, and it is the majority form across the repo at 595 of ~680 looks), and the 👑 heading is a declared per-character field. **And the one real format split turned out to be a parser constraint, not sloppiness:** Ele's gallery cannot host a `**BLOQUE B …:**` + ```` ```text ```` block — measured, it makes LV-App ingest 8 prompts where there are 7 and hands `Standing` two, silently losing a take. She declares her outfit in a one-line `- **Outfit (BLOQUE B):**` field instead. Full parity scorecard, with what must be identical and what may legitimately differ: `.agent/rules/11-contrato-galeria.md` §9quinquies.
...
**BLOQUE B can now be declared as a typed manifest instead of hand-written prose (09/09/2026).** The doctrine, stated by the Ama the same day: *"the prompt has to make Gemini do what it says, with no room for interpretation, and with maximum detail — build the outfit engine from that."* Eleven real engine defects were found and fixed that day, five sharing one root cause: the engine reads free text with regex to guess what a look wears, and "no corset" contains the word "corset" just as much as "corset" does. Two independent Fable audits (one briefed on the day's findings, one given the problem cold, neither seeing the other) converged on the same fix: stop reading prose, declare the categorical facts as data, and render the prose *from* the data instead. Each garment piece in a manifest carries both **categorical fields** (`pieza`/`color`/`estampado_animal`/`cobertura`, closed vocabulary in `99_Sistema/scripts/visual/vocabulario_vestuario.json`, canon the Ama edits) and one free-text `descripcion` — the bespoke construction detail a human still writes. `vestuario_renderer.py` turns a manifest into the BLOQUE B string; `PromptBuilder.opt_in_de()`/`animal_print_kind()`/`build()` read the declared facts directly when a manifest is present, never guessing — and fall back to the (now ausencia-safe) regex path unchanged for the ~1,400 historical looks, which are never migrated. Verified against a real look end-to-end (Look 833, Ele) through the real `outfit.py generar` gate, with 3 of 7 real photos generated and confirmed correct. **Ama's verdict: the manifest performs as well as hand-written prose** — it is the recommended path for new looks, never a retrofit of the past. Full architecture, the two contradiction families, and the auditor built to catch the next one before a photo does: `.agent/skills/outfit-engine/SKILL.md` §"Doctrina del motor" and §"El manifiesto tipado (v4.0)"; evidence with `archivo:línea` citations for all 11 findings: `99_Sistema/auditoria_pares_anclas_20260909.md`.

...
| `prompt_builder.py` | `scripts/visual/` | Ensamblador de prompts del `outfit-engine`, agnóstico de personaje: `PromptBuilder(slug).build(A, B, slot, pose, setting)` y `.build_negative(base)`. Lee `anclas_universales.json`; nunca copia su texto |
| `anclas_universales.json` | `scripts/visual/` | **Dueño único** del texto literal de las 16 anclas anti-defecto, el negative universal, el **significado de los 7 slots de cámara** (Ditzy = waist-up con una mano y mirada fuera del lente · POV = retrato IG mirando al lente), el registro de personajes con sus overrides, y la deuda declarada por personaje |

...
| `pose_rotation_v5.py` | `scripts/visual/` | Rota las 7 poses por look + `SINGLE_FRAME` anti-collage prepuesto ×7 + anclas (anatómica, frontalidad Standing, peso Seated, recumbencia+anti-percha+cámara nivelada Odalisca, prenda envolvente `wrap_mode`, costura de media `seam`, eco de calzado `shoe_echo`) + candados v2 sin metalenguaje multi-toma (`SKIN_LOCK` afirmativo, `UNMARKED_ZONES`, `NO_ARMWEAR`, `OPAQUE_LOCK`, `GLOSS_LOCK`, `CONSISTENCY_LOCK`, `HOSIERY_LOCK`) + **`build_negative()` = fuente única del negative** — lo usa **todo** inyector |

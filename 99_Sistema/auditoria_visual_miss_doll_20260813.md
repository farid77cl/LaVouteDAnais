...
| 🔴 **CORREGIDO 13/08 — mi primera medición estaba mal planteada** | Medí **pose + setting juntos** y salió 10-28%, y de ahí concluí que Miss Doll no tenía el problema de Anaïs. **Falso.** Lo que varía es el **escenario** (club / gym / estudio blanco / calabozo / ventanal), y arrastraba el número hacia abajo escondiendo que **la pose se repite**. Medida la **cláusula de pose sola**, sin anclas y sin setting: **Standing 68% · Side Profile 70% · Odalisque 54% · Seated 53% · POV 52% · Back View 41% · Glacial Command 21%**. En 4 de 5 Standing muestreados la pose es literalmente la misma —`contrapposto, weight shifted onto one hip`— con el adjetivo cambiado (*cruel aggressive* / *confident athletic* / *sharp architectural*). **El único slot sano es el 21%, y es exactamente el único con repertorio escrito.** El propio `repertorio_camara_miss_doll.md` ya lo decía en su §PENDIENTE; le creí a mi métrica en vez de leerlo |
...
| ⏳ **Abierto tras la corrección** | Completar `repertorio_camara_miss_doll.md`: hoy solo tiene los slots **5 y 6** (7 variaciones cada uno); faltan **Standing, Back View, Seated, Side Profile y Odalisque**. Referencia: el motor de Ele (`pose_rotation_v5.py`) lleva **51 sub-poses** repartidas en los 7 slots, en código y con rotación automática |

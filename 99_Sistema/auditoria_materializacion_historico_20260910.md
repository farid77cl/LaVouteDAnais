# Histórico de materialización — archivado 10/09/2026

> Auditoría externa (Fable, ciega) sobre `/inicio-ele` midió `.agent/rules/09-estado-materializacion.md` en 44 KB para ~10 líneas de valor real vivo — el resto era bitácora de incidentes ya cerrados (algunos de julio), tablas de flota de Anaïs/Miss Doll congeladas meses atrás (siguen diciendo "Look 01-40" con la flota real en L94/L95 — la misma clase de contador copiado-y-divergido que el linter H9 existe para cazar) y hallazgos re-medidos varias veces en el mismo documento. Ese archivo se lee EN CADA `/inicio-ele`. Este archivo es la evidencia completa, con fecha, nunca se borra (regla 12). El rule 09 nuevo solo lleva los pendientes vivos + las lecciones permanentes + el flujo operativo.

Contenido íntegro del archivo antes del recorte:
